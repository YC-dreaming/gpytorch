...
class GridInducingVariationalGP(AbstractVariationalGP):
    ...
    def __init__(self, grid_size, grid_bounds):
        ...
        inducing_points = torch.zeros(
            int(pow(grid_size, len(grid_bounds))), len(grid_bounds)
        )
        prev_points = None
        for i in range(len(grid_bounds)):
            for j in range(grid_size):
                inducing_points[j * grid_size ** i : (j + 1) * grid_size ** i, i].fill_(
                    grid[i, j]
                )
    ...
    def __call__(self, inputs, **kwargs):
        ...
        test_mean = left_interp(
            interp_indices, interp_values, variational_output.mean().unsqueeze(-1)
        )
        test_mean = test_mean.squeeze(-1)

        # Compute test covar
        test_covar = InterpolatedLazyVariable(
            variational_output.covar(),
            interp_indices,
            interp_values,
            interp_indices,
            interp_values,
        ...
        if beta_features.diagonal_correction.on():
            ...
            diagonal_correction = DiagLazyVariable(
                (self.covar_diag(inputs) - prior_covar.diag()) * 0
            )
